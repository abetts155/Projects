...
public class CFGGenerator
{
	...
	public CFGGenerator ()
	{
		...
		// addBranchComponents
		// (MainProgramGenerator.Globals.getNumberOfVerticesInCFG(), 4);
		addLoops ();
		// addSelfLoops ();
		...
		Debug.debugMessage (getClass (), "Adding edge " + cfg.getExitID () + "=>"
				+ cfg.getExitID (), 4);
	...
	private void addNonBranchComponents (int n)
	{
		...
		for (int i = 0; i < disconnectedVertices.size () - 1; ++i)
		{
			...
			cfg.addEdge (disconnectedVertices.get (i), disconnectedVertices.get (i + 1),
					BranchType.TAKEN);
		}
	}
	...
	private void buildIndividualIfElseBlocks (int noOfComponents)
	{
		for (int i = 1; i <= noOfComponents; ++i)
		{
			...
			Debug.debugMessage (getClass (), "Adding edge " + branchID + "=>" + ifBranchID, 4);
			cfg.addEdge (branchID, ifBranchID, BranchType.TAKEN);
			Debug.debugMessage (getClass (), "Adding edge " + branchID + "=>" + elseBranchID, 4);
			cfg.addEdge (branchID, elseBranchID, BranchType.TAKEN);
			Debug.debugMessage (getClass (), "Adding edge " + ifBranchID + "=>" + mergeID, 4);
			cfg.addEdge (ifBranchID, mergeID, BranchType.TAKEN);
			Debug.debugMessage (getClass (), "Adding edge " + elseBranchID + "=>" + mergeID, 4);
		...
		for (int i = 0; i < disconnectedBranches.size () - 3; i = i + 2)
		{
			if (random.nextBoolean ())
			{
				if (destinationToRemove >= 0)
				{
					...
					int destinationRemovedID = cfg.getVertex (sourceID).getNthSuccessor (
							destinationToRemove).getVertexID ();
			...
			} else
			{
				...
				cfg.addEdge (disconnectedBranches.get (i + 1), disconnectedBranches.get (i + 2),
						BranchType.TAKEN);
			}
		}
	}
	...
	private void addLoops ()
	{
		...
		else
		{
			...
			Debug.debugMessage (getClass (), "Adding edge " + entryID + "=>"
					+ disconnectedLoopsArray.get (0), 4);
			cfg.addEdge (entryID, disconnectedLoopsArray.get (0), BranchType.TAKEN);
			Debug.debugMessage (getClass (), "Adding edge " + disconnectedLoopsArray.get (0) + "=>"
					+ exitID, 4);
	...
	private void buildIndividualLoops (int loops)
	{
		for (int i = 0; i < loops; ++i)
		{
			...
			if (remainingVertices > 2)
			{
				...
				boolean check = (type == 3 && loopBody < 3) || (type == 4 && loopBody < 4)
						|| random.nextBoolean ();
				...
				if (check)
				{
					...
					Debug.debugMessage (getClass (), "Adding edge "
							+ disconnectedVertices.get (disconnectedVertices.size () - 1) + "=>"
							+ tailID, 4);
					cfg.addEdge (disconnectedVertices.get (disconnectedVertices.size () - 1),
							tailID, BranchType.TAKEN);
				} else
				{
					Debug.debugMessage (getClass (), "Adding edge " + headerID + "=>"
					...
					Debug.debugMessage (getClass (), "Adding edge "
							...
							+ tailID, 4);
					cfg.addEdge (disconnectedBranches.get (disconnectedBranches.size () - 1),
							tailID, BranchType.TAKEN);
				}
			} else
			{
	...
	private void connectLoops ()
	{
		for (int i = disconnectedLoops.getHeight () - 1; i > 0; --i)
		{
			...
			cfg.addEdge (childID, cfg.getVertex (parentID).getNthSuccessor (0).getVertexID (),
					BranchType.TAKEN);
			...
			while (it.hasNext ())
			{
				...
				if (parentID == nextParentID)
				{
					Debug.debugMessage (getClass (),
							"Adding edge " + parentID + "=>" + nextChildID, 4);
					cfg.addEdge (parentID, nextChildID, BranchType.TAKEN);
					Debug.debugMessage (getClass (), "Adding edge " + nextChildID + "=>" + childID,
							4);
					cfg.addEdge (nextChildID, childID, BranchType.TAKEN);
					Debug.debugMessage (getClass (), "Removing edge " + parentID + "=>" + childID,
							4);
				...
				} else
				{
					...
					cfg.addEdge (nextChildID, cfg.getVertex (nextParentID).getNthSuccessor (0)
							.getVertexID (), BranchType.TAKEN);
					Debug.debugMessage (getClass (), "Removing edge " + nextParentID + "=>"
							+ cfg.getVertex (nextParentID).getNthSuccessor (0).getVertexID (), 4);
					cfg.removeEdge (nextParentID, cfg.getVertex (nextParentID).getNthSuccessor (0)
							.getVertexID ());
	...
	private void checkLoopHeaders ()
	{
		for (Vertex header : disconnectedLoops)
		{
			if (header.numOfSuccessors () < 2)
			{
				...
				Debug.debugMessage (getClass (), "Adding edge " + header.getVertexID () + "=>"
						+ failCaseID, 4);
	...
	private void setEntry ()
	{
		...
		{
			Debug.debugMessage (getClass (),
					"Unable to find entry vertex : too many vertices with no predecessors.", 1);
			System.exit (1);
		} else if (noPreds.size () == 0)
		{
			Debug.debugMessage (getClass (),
					"Unable to find entry vertex : no vertex without predecessors found. ", 1);
			System.exit (1);
		} else
		{
	...
	private void setExit ()
	{
		...
		{
			Debug.debugMessage (getClass (),
					"Unable to find exit vertex : too many vertices with no predecessors.", 1);
			System.exit (1);
		} else if (noSuccs.size () == 0)
		{
			Debug.debugMessage (getClass (),
					"Unable to find exit vertex : no vertex without successors found. ", 1);
			System.exit (1);
		} else
		{
		...
		while (noPreds.size () > 1)
		{
			...
			for (int i = 1; i <= MainProgramGenerator.Globals.getFanOut (); ++i)
			{
				if (!noPreds.isEmpty ())
				{
					...
					Debug.debugMessage (getClass (), "...and connecting it to " + v.getVertexID (),
							4);
			...
			while (!noSuccs.isEmpty ())
			{
				...
				Debug
						.debugMessage (getClass (),
								"...and connecting it from " + v.getVertexID (), 4);
