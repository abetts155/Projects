...
public class CFGGenerator
{
    ...
    public CFGGenerator ()
    {
        remainingVertices = MainProgramGenerator.Globals
                .getNumberOfVerticesInCFG();

        Debug.debugMessage(getClass(), "#Vertices in CFG = "
                + remainingVertices, 1);
            ...
            while (levelIt.hasNext())
            {
                ...
                if (level == 0)
                {
                    ...
                    Debug.debugMessage(getClass(),
                            "#Vertices in loop " + treev.getVertexID() + " = "
    ...
    private LoopComponent setLoopVertices (
            SingleEntrySingleExitComponent seseComponent)
    {
        ...
        if (random.nextBoolean()
                && cfg.getVertex(seseComponent.startID).numOfSuccessors() == 1)
        {
        ...
        Debug.debugMessage(getClass(), "Header = " + loopComponent.headerID
                + ", tails = " + loopComponent.tails + ", exits = "
                + loopComponent.exits, 4);
    ...
    {
        Debug.debugMessage(getClass(),
                "Adding nested loops for " + treev.getVertexID(), 1);
        ...
        if (successors.size() > 1)
        {
            ...
            while (successors.size() > 1)
            {
                ...
                {
                    cfg.addEdge(exitID, succLoopComponent.headerID,
                            BranchType.TAKEN);
                }
            }
    ...
    private Tree buildLNT ()
    {
        ...
        for (Vertex v : tree)
        {
            ...
            if (vertexID != rootID)
            {
                ...
                if (newLevel <= MainProgramGenerator.Globals
                        .getLoopNestingLevelDepth())
                {
                ...
                else
                {
                    ...
                    tree.addEdge(parentID, vertexID);
                    vertexToLevel
                            .put(vertexID, vertexToLevel.get(parentID) + 1);
                    parentID = vertexID;
                }
            }
        }
    ...
    private void decideWhichAcyclicComponents ()
    {
        if (random.nextBoolean())
        {
            ...
            Debug.debugMessage(getClass(),
                    "Number of if-then-else components = "
        ...
        if (random.nextBoolean())
        {
            ...
            Debug.debugMessage(getClass(),
                    "Number of short circuited logical and components = "
                            + numberOfShortCircuitedAndComponents, 1);
        }

        if (MainProgramGenerator.Globals.getFanOut() > 2
                && random.nextBoolean())
        {
            final int sizeOfComponent = 2 + MainProgramGenerator.Globals
                    .getFanOut();
    ...
    private int setNumberOfComponents (final int sizeOfComponent)
    {
        ...
        if (remainingVerticesInRegion >= sizeOfComponent)
        {
            numOfComponents = random.nextInt((int) Math
                    .floor(remainingVerticesInRegion / sizeOfComponent)) + 1;

            remainingVerticesInRegion = remainingVerticesInRegion
                    - (numOfComponents * sizeOfComponent);

            Debug.debugMessage(getClass(), "Component size = "
                    + sizeOfComponent + ". #Components = " + numOfComponents
                    + ". #Remaining vertices = " + remainingVerticesInRegion, 4);
        }
    ...
    private void addAcyclicComponents ()
    {
        while (numberOfIfThenElseComponents > 0 || numberOfIfThenComponents > 0
                || numberOfShortCircuitedAndComponents > 0
                || numberOfCaseComponents > 0)
        {
    ...
    private SingleEntrySingleExitComponent addShortCircuitedAndComponent ()
    {
        ...
        Debug.debugMessage(getClass(), "Adding outer branch vertex "
                + outerBranchID, 4);
        cfg.addBasicBlock(outerBranchID);

        int innerBranchID = cfg.getNextVertexID();
        Debug.debugMessage(getClass(), "Adding inner branch vertex "
                + innerBranchID, 4);
    ...
    private SingleEntrySingleExitComponent buildSESEComponent ()
    {
        ...
        else if (disconnectedComponents.size() > 0 && random.nextBoolean())
        {
            ...
            cfg.addEdge(seseComponent.getExitID(),
                    nestedSeseComponent.getStartID(), BranchType.TAKEN);
    ...
    private void connectDisconnectedComponents ()
    {
        ...
        {
            SingleEntrySingleExitComponent firstSeseComponent = disconnectedComponents
                    .remove(0);
            SingleEntrySingleExitComponent secondSeseComponent = disconnectedComponents
                    .remove(0);
            ...
            cfg.addEdge(sourceID, secondSeseComponent.getStartID(),
                    BranchType.TAKEN);
    ...
    {
        Debug.debugMessage(getClass(),
                "Finding potential merge vertices to remove", 1);
        ...
        {
            Debug.debugMessage(getClass(),
                    "Analysing merge vertex " + vertexID, 2);

            HashSet <Integer> predIDs = cfg.getVertex(vertexID)
                    .getPredecessorIDs();
            HashSet <Integer> succIDs = cfg.getVertex(vertexID)
                    .getSuccessorIDs();
        ...
        {
            Debug.debugMessage(getClass(),
                    "Updating new successors of unlinked vertices", 2);
            ...
            for (int predID : newSuccessors.keySet())
            {
                ...
                {
                    if (toRemove.contains(succID)
                            && newSuccessors.containsKey(succID))
                    {
                        newSuccessors.get(predID).remove(succID);
                        newSuccessors.get(predID).addAll(
                                newSuccessors.get(succID));

                        Debug.debugMessage(getClass(), "Updated: newSucc("
                                + predID + ")" + newSuccessors.get(predID), 4);
        ...
        for (int vertexID : toRemove)
        {
            Debug.debugMessage(getClass(), "Removing merge vertex " + vertexID,
                    2);
        ...
        for (int predID : newSuccessors.keySet())
        {
            ...
            {
                Debug.debugMessage(getClass(),
                        "Updating new successors of vertex " + predID, 3);
    ...
    private SingleEntrySingleExitComponent connectRemainingVertices ()
    {
        ...
        {
            Debug.debugMessage(getClass(),
                    "Disconnected components to connect", 2);

            SingleEntrySingleExitComponent seseComponent = disconnectedComponents
                    .remove(0);
            ...
            while (remainingVerticesInRegion > 0)
            {
                ...
                if (random.nextBoolean())
                {
                    cfg.addEdge(vertexID, seseComponent.getStartID(),
                            BranchType.TAKEN);
                    seseComponent.setStartID(vertexID);
                }
                else
                {
                    cfg.addEdge(seseComponent.getExitID(), vertexID,
                            BranchType.TAKEN);
                    seseComponent.setExitID(vertexID);
                }
        ...
        Debug.debugMessage(getClass(), "SESE entry = "
                + masterSeseComponent.startID + ". SESE exit = "
    ...
    private void setExit ()
    {
        ...
        if (noSuccs.size() > 1)
        {
            OutputGraph.output(cfg);
            Debug.errorMessage(getClass(),
                    "Unable to find exit vertex. Too many vertices with no successors: "
                            + noSuccs);
        }
    ...
    private void addSelfLoops ()
    {
                ...
                && selfLoopCandidates.size() > 0)
        {
            ...
            Debug.debugMessage(getClass(), "Self-loop candidates = "
                    + selfLoopCandidates, 4);
    ...
    private void addBreaksAndContinues ()
    {
    ...
    private void addBreaks (HeaderVertex headerv, LoopNests lnt)
    {
        ...
        Debug.debugMessage(getClass(), "Trying to add breaks to " + headerID, 2);

        ArrayList <Integer> candidateSources = new ArrayList <Integer>();
        for (int bbID : lnt.body(headerID))
        {
            if (bbID != headerID && lnt.isLoopTail(headerID, bbID) == false
                    && lnt.isLoopHeader(bbID) == false)
            {
                Vertex v = cfg.getVertex(bbID);
