...
public class IpointGraph extends FlowGraph
{
	...
	public IpointGraph (CFGStar cfgStar, LoopNests lnt)
	{
			...
			public void doJob ()
			{
				for (int headerID : auxiliaryData.hToTopSorts.keySet ())
				{
					Debug.debugMessage (IpointGraph.class, "topsort(" + headerID + ") = "
							+ auxiliaryData.hToTopSorts.get (headerID), 4);
				}
			}
		}, 4);
		...
		Debug.debugMessage (new CallBack ()
		{
			...
			public void doJob ()
			{
				...
				{
					for (int keyID : auxiliaryData.vToReachable.get (vertexID).keySet ())
					{
						Debug.debugMessage (IpointGraph.class, "Path(" + keyID + " => " + vertexID
								+ ") = " + auxiliaryData.vToReachable.get (vertexID).get (keyID), 4);
					}
				}
			}
		}, 4);
	...
	public IpointGraph (CFGStar cfgStar)
	{
		...
		Debug.debugMessage (getClass (), "Adding edges using data-flow analysis", 2);
	...
	public Ipoint getVertex (long ipointID)
	{
		...
		assert returnv != null : "Cannot find Ipoint with ID 0x" + Long.toHexString (ipointID);
		return returnv;
	}

	public boolean hasIpointID (long ipointID)
	{
		Debug.debugMessage (getClass (), "Looking for Ipoint " + Long.toHexString (ipointID), 4);
		for (Vertex v : this)
		{
			Ipoint u = (Ipoint) v;
			/*
	...
	private void initialise (AuxiliaryData auxiliaryData)
	{
		...
		{
			int vertexID = v.getVertexID ();
			auxiliaryData.vToReachable.put (vertexID, new HashMap <Integer, HashSet <Integer>> ());
			...
			if (cfgStar.isIpoint (vertexID))
			{
				...
				} else
				{
					...
					auxiliaryData.ipointToHeader.put (vertexID, headerv.getHeaderID ());
				}
			}
			...
			if (lnt.isLoopHeader (vertexID))
			{
				if (!lnt.isSelfLoop (vertexID))
				{
					...
					auxiliaryData.hToTopSorts.put (vertexID, new ArrayList <Integer> ());
					auxiliaryData.hToIterationEdges.put (vertexID, new HashSet <Integer> ());
					auxiliaryData.hToIpoints.put (vertexID, new HashSet <Integer> ());
	...
	private void doDFS (AuxiliaryData auxiliaryData, Set <Integer> visited, int vertexID)
	{
		...
		if (vertexID != rootv.getHeaderID ())
		{
			...
			HeaderVertex headerv = (HeaderVertex) lnt.getVertex (headerID);
			ArrayList <Integer> topSort = auxiliaryData.hToTopSorts.get (headerv.getHeaderID ());
			topSort.add (0, vertexID);

			Debug.debugMessage (getClass (),
					"Adding " + vertexID + " to " + headerv.getHeaderID (), 4);

			if (lnt.isLoopHeader (vertexID))
			{
				int enclosingHeaderID = headerv.getParentID ();
				HeaderVertex enclosingHeaderv = (HeaderVertex) lnt.getVertex (enclosingHeaderID);
				...
				Debug.debugMessage (getClass (), "Adding " + vertexID + " to enclosing header "
						+ enclosingHeaderv.getHeaderID (), 4);
			}
		} else
		{
			ArrayList <Integer> topSort = auxiliaryData.hToTopSorts.get (vertexID);
			topSort.add (0, vertexID);
		}
	}
	...
	private void addEdges (AuxiliaryData auxiliaryData)
	{
		for (int level = lnt.getHeight () - 1; level >= 0; --level)
		{
			...
			while (levelIt.hasNext ())
			{
				...
				{
					Debug.debugMessage (getClass (), "Building IPG transitions in CFG* header "
							+ vertexID, 3);
					...
					if (!cfgStar.isIpoint (vertexID))
					{
						...
						path.add (vertexID);
						auxiliaryData.vToReachable.get (vertexID).put (vertexID, path);
					}
		...
		while (auxiliaryData.changed)
		{
			...
			for (int vertexID : auxiliaryData.hToTopSorts.get (headerID))
			{
				...
				while (predIt.hasNext ())
				{
					...
					{
						if (lnt.isLoopTail (headerID, predID) && auxiliaryData.it > 1)
						{
							analyseEdge = true;
						}
					...
					if (analyseEdge)
					{
						Debug.debugMessage (getClass (), "It " + auxiliaryData.it + ": CFG* edge "
								+ predID + " => " + vertexID, 4);

						if (cfgStar.isIpoint (predID))
						{
							update (auxiliaryData, predID, vertexID, headerID, predID);
						} else
						{
							for (int keyID : auxiliaryData.vToReachable.get (predID).keySet ())
							{
								if (cfgStar.isIpoint (keyID))
								{
									update (auxiliaryData, keyID, vertexID, headerID, predID);
								}
							}

							if (auxiliaryData.it == 1
									&& auxiliaryData.vToReachable.get (predID).containsKey (
											headerID))
							{
								updateLoopReachability (auxiliaryData, headerID, vertexID, predID);
							}
						}
					}
				}
	...
	private void update (AuxiliaryData auxiliaryData, int uID, int vID, int headerID, int predID)
	{
		...
		if (cfgStar.isIpoint (vID))
		{
			if (!u.hasSuccessor (vID))
			{
				...
				if (predID != uID)
				{
					addEdgeLabel (auxiliaryData, uID, vID, auxiliaryData.vToReachable.get (predID)
							.get (uID));
				}
		...
		{
			if (lnt.isLoopHeader (vID) && !lnt.isSelfLoop (vID) && vID != headerID)
			{
			...
			} else
			{
				if (!auxiliaryData.vToReachable.get (vID).containsKey (uID))
				{
					...
					if (predID != uID)
					{
						path.addAll (auxiliaryData.vToReachable.get (predID).get (uID));
					}

					Debug.debugMessage (getClass (), "Added " + path + " to the path " + uID
							+ " to " + vID, 4);
					auxiliaryData.vToReachable.get (vID).put (uID, path);
				} else if (predID != uID)
				{
					int oldSize = auxiliaryData.vToReachable.get (vID).get (uID).size ();
					auxiliaryData.vToReachable.get (vID).get (uID)
							.addAll (auxiliaryData.vToReachable.get (predID).get (uID));
					int newSize = auxiliaryData.vToReachable.get (vID).get (uID).size ();

					if (oldSize != newSize)
					{
						Debug.debugMessage (getClass (),
								"Added " + auxiliaryData.vToReachable.get (predID).get (uID)
	...
	private void addIPGEdge (AuxiliaryData auxiliaryData, int sourceID, int destinationID)
	{
		Debug.debugMessage (getClass (), "Adding edge " + sourceID + " => " + destinationID, 3);

		Ipoint u = (Ipoint) idToVertex.get (sourceID);
		Ipoint v = (Ipoint) idToVertex.get (destinationID);

		if (sourceID == entryID || destinationID == exitID || sourceID == exitID)
		{
			u.addSuccessor (destinationID, v.getIpointID (), auxiliaryData.edgeID,
					IPGEdgeType.GHOST_EDGE);
			v.addPredecessor (sourceID, auxiliaryData.edgeID, IPGEdgeType.GHOST_EDGE);
		} else if (u.isInlinedEntry () && v.isInlinedExit ())
		{
			u.addSuccessor (destinationID, v.getIpointID (), auxiliaryData.edgeID,
					IPGEdgeType.INLINED_EDGE);
			v.addPredecessor (sourceID, auxiliaryData.edgeID, IPGEdgeType.INLINED_EDGE);
		} else
		{
			u.addSuccessor (destinationID, v.getIpointID (), auxiliaryData.edgeID,
					IPGEdgeType.TRACE_EDGE);
			v.addPredecessor (sourceID, auxiliaryData.edgeID, IPGEdgeType.TRACE_EDGE);
		}
	...
	private void addEdgeLabel (AuxiliaryData auxiliaryData, int sourceID, int destinationID,
			HashSet <Integer> label)
	{
		Ipoint u = (Ipoint) idToVertex.get (sourceID);
		Ipoint v = (Ipoint) idToVertex.get (destinationID);
		...
		if (oldSize != newSize)
		{
			Debug.debugMessage (getClass (), "Added " + label + " to the IPG edge " + sourceID
					+ " => " + destinationID, 4);
			auxiliaryData.changed = true;
		}
	}

	private void setIterationEdge (AuxiliaryData auxiliaryData, int sourceID, int destinationID)
	{
		Debug.debugMessage (getClass (), "Iteration edge: " + sourceID + " => " + destinationID
				+ " for " + auxiliaryData.headerID, 3);
	...
	private void determineAcyclicProperties (AuxiliaryData auxiliaryData, int sourceID,
			int destinationID)
	{
		Debug.debugMessage (getClass (), "Determining acyclic properties of " + sourceID + " => "
				+ destinationID, 3);
		...
		int destinationHeaderID = auxiliaryData.ipointToHeader.get (destinationID);

		HeaderVertex sourceHeaderv = (HeaderVertex) lnt.getVertex (lnt.getVertex (sourceHeaderID)
				.getParentID ());
		HeaderVertex destinationHeaderv = (HeaderVertex) lnt.getVertex (lnt.getVertex (
				destinationHeaderID).getParentID ());

		if (sourceHeaderv.getHeaderID () != destinationHeaderv.getHeaderID ())
		{
			if (lnt.isNested (sourceHeaderv.getVertexID (), destinationHeaderv.getVertexID ()))
			{
	...
	private void addLoopEntryEdges (AuxiliaryData auxiliaryData, int sourceID, int headerID,
			int predID)
	{
		Debug.debugMessage (getClass (), "Adding loop entry edges of " + headerID, 3);
		...
		for (int ipointID : auxiliaryData.hToReachable.get (headerID).keySet ())
		{
			if (!u.hasSuccessor (ipointID))
			{
				...
				if (predID != sourceID)
				{
					path.addAll (auxiliaryData.vToReachable.get (predID).get (sourceID));
				}
				path.addAll (auxiliaryData.hToReachable.get (headerID).get (ipointID));
				...
				{
					determineAcyclicProperties (auxiliaryData, sourceID, ipointID);
				}
			} else
			{
				if (auxiliaryData.it == 2)
	...
	private void updateLoopReachability (AuxiliaryData auxiliaryData, int headerID, int vertexID,
			int predID)
	{
		Debug.debugMessage (getClass (), "Updating loop " + headerID + " from " + vertexID, 3);
		...
		if (cfgStar.isIpoint (vertexID))
		{
			if (!auxiliaryData.hToReachable.get (headerID).containsKey (vertexID))
			{
				auxiliaryData.changed = true;
				HashSet <Integer> path = new HashSet <Integer> ();
				path.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
			...
			{
				int oldSize = auxiliaryData.hToReachable.get (headerID).get (vertexID).size ();
				auxiliaryData.hToReachable.get (headerID).get (vertexID)
						.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
				int newSize = auxiliaryData.hToReachable.get (headerID).get (vertexID).size ();

				if (oldSize != newSize)
				{
					Debug.debugMessage (getClass (),
							"Added " + auxiliaryData.vToReachable.get (predID).get (headerID)
									+ " to the path " + headerID + " to " + vertexID, 4);
					auxiliaryData.changed = true;
				}
			}
		} else
		{
			if (lnt.isLoopHeader (vertexID) && !lnt.isSelfLoop (vertexID) && vertexID != headerID)
			{
				updateLoopExits (auxiliaryData, vertexID, headerID, predID);

				for (int ipointID : auxiliaryData.hToReachable.get (vertexID).keySet ())
				{
					if (!auxiliaryData.hToReachable.get (headerID).containsKey (ipointID))
					{
						auxiliaryData.changed = true;

						HashSet <Integer> path = new HashSet <Integer> ();
						path.addAll (auxiliaryData.hToReachable.get (vertexID).get (ipointID));
						path.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
						auxiliaryData.hToReachable.get (headerID).put (ipointID, path);
					} else
					{
						int oldSize = auxiliaryData.hToReachable.get (headerID).get (ipointID)
								.size ();
						auxiliaryData.hToReachable.get (headerID).get (ipointID)
								.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
						int newSize = auxiliaryData.hToReachable.get (headerID).get (ipointID)
								.size ();
			...
			{
				if (!auxiliaryData.vToReachable.get (vertexID).containsKey (headerID))
				{
					auxiliaryData.changed = true;
					...
					path.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
					path.add (vertexID);
					auxiliaryData.vToReachable.get (vertexID).put (headerID, path);
				} else
				{
					int oldSize = auxiliaryData.vToReachable.get (vertexID).get (headerID).size ();
					auxiliaryData.vToReachable.get (vertexID).get (headerID)
							.addAll (auxiliaryData.vToReachable.get (predID).get (headerID));
					int newSize = auxiliaryData.vToReachable.get (vertexID).get (headerID).size ();
					if (oldSize != newSize)
					{
						auxiliaryData.changed = true;
					}
	...
	private void updateLoopExits (AuxiliaryData auxiliaryData, int headerID, int keyID, int predID)
	{
		Debug.debugMessage (getClass (), "Updating exits in " + headerID + " with key " + keyID, 3);
		...
		while (exitIt.hasNext ())
		{
			...
			if (auxiliaryData.vToReachable.get (exitID).containsKey (headerID))
			{
				if (!auxiliaryData.vToReachable.get (exitID).containsKey (keyID))
				{
					...
					if (predID != keyID)
					{
						path.addAll (auxiliaryData.vToReachable.get (predID).get (keyID));
					}
					path.addAll (auxiliaryData.vToReachable.get (exitID).get (headerID));
					auxiliaryData.vToReachable.get (exitID).put (keyID, path);
				} else if (predID != keyID)
				{
					int oldSize = auxiliaryData.vToReachable.get (exitID).get (keyID).size ();
					auxiliaryData.vToReachable.get (exitID).get (keyID)
							.addAll (auxiliaryData.vToReachable.get (predID).get (keyID));
					int newSize = auxiliaryData.vToReachable.get (exitID).get (keyID).size ();
	...
	{
		Debug.debugMessage (getClass (), "Collapsing inner loops in " + headerID, 3);
		...
		while (succIt.hasNext ())
		{
			...
			{
				HashSet <Integer> ipoints = auxiliaryData.hToIpoints.get (succID);
	...
	private void addEdgesThroughDataFlow ()
	{
		...
		{
			int vertexID = v.getVertexID ();
			vToIpoints.put (vertexID, new HashMap <Integer, HashSet <Integer>> ());
		}
			...
			for (int i = dfs.numOfVertices (); i >= 1; --i)
			{
				...
				while (predIt.hasNext ())
				{
					...
					if (cfgStar.isIpoint (predID))
					{
						...
						{
							changed = true;
							vToIpoints.get (vertexID).put (predID, new HashSet <Integer> ());

							if (!cfgStar.isIpoint (vertexID))
							{
								vToIpoints.get (vertexID).get (predID).add (vertexID);
							}
						}
					} else
					{
						...
						for (int keyID : vToIpoints.get (predID).keySet ())
						{
							...
							{
								changed = true;
								vToIpoints.get (vertexID).put (keyID, new HashSet <Integer> ());

								Set <Integer> pathFromKeyToPred = vToIpoints.get (predID).get (
										keyID);
								vToIpoints.get (vertexID).get (keyID).addAll (pathFromKeyToPred);

								if (!cfgStar.isIpoint (vertexID))
								{
									vToIpoints.get (vertexID).get (keyID).add (vertexID);
								}
							} else
							{
								int oldSize = vToIpoints.get (vertexID).get (keyID).size ();
								Set <Integer> pathFromKeyToPred = vToIpoints.get (predID).get (
										keyID);
								vToIpoints.get (vertexID).get (keyID).addAll (pathFromKeyToPred);
								int newSize = vToIpoints.get (vertexID).get (keyID).size ();
		...
		for (Vertex cfgv : cfgStar)
		{
			...
			if (cfgStar.isIpoint (destinationID))
			{
				for (int sourceID : vToIpoints.get (destinationID).keySet ())
				{
					Ipoint u = (Ipoint) idToVertex.get (sourceID);
					Ipoint v = (Ipoint) idToVertex.get (destinationID);
					u.addSuccessor (destinationID, v.getIpointID (), edgeID, IPGEdgeType.GHOST_EDGE);
					v.addPredecessor (sourceID, edgeID, IPGEdgeType.GHOST_EDGE);

					IPGEdge succEdge = (IPGEdge) u.getSuccessor (destinationID);
					IPGEdge predEdge = (IPGEdge) v.getPredecessor (sourceID);

					HashSet <Integer> label = vToIpoints.get (destinationID).get (sourceID);
