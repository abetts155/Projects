...
public class ControlFlowGraph extends FlowGraph implements Cloneable
{
		...
		if (renumberingRequired ())
		{
			...
			for (Vertex v : this)
			{
				...
				while (succIt.hasNext ())
				{
					...
					Debug.debugMessage (getClass (), "Renumbering succ " + oldSuccID + " to "
							+ newSuccID, 4);
				}
				...
				while (predIt.hasNext ())
				{
					...
					Debug.debugMessage (getClass (), "Renumbering pred " + oldPredID + " to "
							+ newPredID, 4);
				}
			}
		}
	...
	{
		assert idToVertex.containsKey (vertexID) : "Cannot find vertex " + vertexID;
