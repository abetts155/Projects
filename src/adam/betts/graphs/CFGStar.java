...
public class CFGStar extends ControlFlowGraph
{
	...
	public CFGStar (ControlFlowGraph cfg, IProfile profile, String subprogramName)
	{
		this.cfg = cfg;
				...
				&& cfg.getExitID () != Vertex.DUMMY_VERTEX_ID)
		{
			...
			{
				Debug.debugMessage (getClass (), "Removing edge " + cfg.getExitID () + " => "
						+ cfg.getEntryID (), 4);
	...
	public final void addIpoint (int vertexID, long ipointID, String subprogramName)
	{
		idToVertex.put (vertexID, new Ipoint (vertexID, ipointID, subprogramName));
	}
	...
	public final void addInline (int siteID, IpointGraph ipg, String calleeName)
	{
		...
		for (Integer entryID : ipg.masterEntries ())
		{
			...
			addIpoint (ipointID, ipg.getVertex (entryID).getIpointID (), calleeName);
			...
			Debug.debugMessage (getClass (), "Adding edge " + ipointID + " => " + siteID, 4);
		}
		...
		for (Integer exitID : ipg.masterExits ())
		{
			...
			Debug.debugMessage (getClass (), "Adding edge " + siteID + " => " + ipointID, 4);
		}
		...
		for (int predID : predIDs)
		{
			...
			Debug.debugMessage (getClass (), "Removing edge " + predID + " => " + siteID, 4);
		}

		for (int succID : succIDs)
		{
			removeEdge (siteID, succID);
			Debug.debugMessage (getClass (), "Removing edge " + siteID + " => " + succID, 4);
		}
	}
	...
	private void instrumentBasicBlocks ()
	{
		...
		for (Vertex v : cfg)
		{
			...
			if (lnt.isLoopHeader (v.getVertexID ()))
			{
				...
				Debug.debugMessage (getClass (), "Adding ipoint " + ipointID + " for bb "
						+ bb.getVertexID (), 4);
			...
			while (succIt.hasNext ())
			{
				...
				if (e.getBranchType () == BranchType.TAKEN)
				{
					...
					if (!instrumented.containsKey (succID))
					{
						...
						Debug.debugMessage (getClass (), "Adding ipoint " + ipointID + " for edge "
								+ v.getVertexID () + "=>" + succID, 4);
						Debug.debugMessage (getClass (), "Adding edge " + ipointID + " => "
								+ succID, 4);
					}
				}
	...
	private void instrumentFunctionLimits (HashMap <Integer, Integer> instrumented)
	{
		for (Vertex v : cfg)
		{
			 ...
			 * single vertex
			 */
			if (bb.numOfPredecessors () == 0 && !instrumented.containsKey (vertexID))
			{
				int ipointID = getNextVertexID ();
				addIpoint (ipointID, bb.getFirstAddress ());
				addEdge (ipointID, vertexID);

				Debug.debugMessage (getClass (), "Adding ipoint " + ipointID + " for entry vertex "
						+ vertexID, 4);
				Debug.debugMessage (getClass (), "Adding edge " + ipointID + " => " + vertexID, 4);
			}
			 ...
			 */
			if (bb.numOfSuccessors () == 0 && !instrumented.containsKey (vertexID))
			{
				int ipointID = getNextVertexID ();
				addIpoint (ipointID, bb.getLastAddress ());
				addEdge (bb.getVertexID (), ipointID);

				Debug.debugMessage (getClass (), "Adding ipoint " + ipointID + " for exit vertex "
						+ vertexID, 4);
				Debug.debugMessage (getClass (), "Adding edge " + bb.getVertexID () + " => "
						+ ipointID, 4);
			}
		}
	}
	...
	private void instrumentPreDominatorLeaves ()
	{
		...
		{
			UDrawGraph.makeUDrawFile (predomT, DominatorTreeType.PRE_DOMINATOR, subprogramName);
		}
		...
		for (Vertex v : predomT)
		{
			...
			if (vertexID != cfg.getExitID () && vertexID != cfg.getEntryID () && treev.isLeaf ())
			{
				int ipointID = getNextVertexID ();
				addIpoint (ipointID, bb.getFirstAddress ());
				addEdge (ipointID, vertexID);
	...
	private void instrumentRandomly ()
	{
		...
		ArrayList <Integer> uninstrumented = new ArrayList <Integer> (idToVertex.keySet ());
