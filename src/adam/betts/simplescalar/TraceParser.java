...
public class TraceParser
{
	...
	public TraceParser (Program program)
	{
		...
		try
		{
			...
			{
				throw new IOException (traceFileName
				...
				} else
				{
					...
					if (c == added)
					{
								...
								&& !lexemes[instrIndex].contains ("ldp"))
						{
							...
							Debug.debugMessage (getClass (), "New instruction with id " + id
									+ " @ address " + lexemes[addressIndex] + " entering pipeline",
									4);
					...
					} else if (c == progress)
					{
						...
						Debug.debugMessage (getClass (), id + " entering stage " + stage, 4);
					} else if (c == retire)
					{
						int id = Integer.parseInt (lexemes[idIndex]);
						Debug.debugMessage (getClass (), id + " is leaving the pipeline ", 4);
	...
	private void decideAction (int id) throws IOException
	{
		...
		if (subprogram != null)
		{
			...
			Debug.debugMessage (getClass (), "In subprogram " + subprogram.getSubprogramName (), 3);
			...
			{
				Debug.debugMessage (getClass (), "Entering " + subprogram.getSubprogramName (), 1);
				callStack.push (subprogram.getSubprogramID ());
			} else if (callStack.peek () != subprogram.getSubprogramID ())
			{
				Debug.debugMessage (getClass (), "Calling " + subprogram.getSubprogramName (), 1);
				callStack.push (subprogram.getSubprogramID ());
			}
			...
			for (IProfile iprofile : Globals.getInstrumentationProfiles ())
			{
				...
				{
					Debug.debugMessage (getClass (), "Ipoint " + address + " found", 4);
					if (MainSimpleScalarTraceParser.outputHexAddresses ())
					{
