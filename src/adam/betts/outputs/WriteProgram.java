...
public class WriteProgram
{
	...
	public WriteProgram (Program program, String fileName, boolean xml)
	{
	...
	private class InHouseOutput
	{
		public InHouseOutput ()
		{
			try
			{
		...
		private void writeCFG (BufferedWriter out, Subprogram subprogram, CallGraph callg)
				throws IOException
		{
			ControlFlowGraph cfg = subprogram.getCFG ();
			out.write ("CFG: " + subprogram.getSubprogramName () + "\n");

			for (Vertex v : cfg)
			{
				BasicBlock bb = (BasicBlock) v;
				out.write ("bb: " + bb.getVertexID () + "\n");
				out.write ("succ: ");
				...
				while (succIt.hasNext ())
				{
					...
					{
						out.write ("(" + subprogram.getSubprogramName () + ","
								+ succEdge.getVertexID () + ") ");
					}
				}
				...
				{
					Subprogram calleeSubprogram = program.getSubprogram (calleeID);
					ControlFlowGraph calleeCFG = calleeSubprogram.getCFG ();
					out.write ("(" + calleeSubprogram.getSubprogramName () + ","
							+ calleeCFG.getEntryID () + ")");
				}
				out.newLine ();
			}
		}
