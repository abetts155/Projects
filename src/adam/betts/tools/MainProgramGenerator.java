...
public class MainProgramGenerator
{
    ...
    private static void addOptions ()
    {
        ...
        fanOutOption = new Option("F", "fan-out", true,
                "Maximum number of successors of a vertex in range ["
                        + minFanOut + ".." + maxFanOut + "]. Default is "
                        + Globals.fanOut + ".");
        fanOutOption.setRequired(false);
        options.addOption(fanOutOption);

        loopsOption = new Option("l", "loops", true,
                "Maximum number of loops in a single graph in range [0.."
                        + maxNumberOfLoops + "]. Default is " + Globals.loops
                        + ".");
        loopsOption.setRequired(false);
        options.addOption(loopsOption);

        loopsDepthOption = new Option("L", "loop-depth", true,
                "Maximum loop-nesting level depth. Default is "
                        + Globals.loopsDepth + ".");
        ...
        selfLoopsOption = new Option("S", "self-loops", true,
                "Maximum number of self-loops in a single graph in range [0.."
                        + maxNumberOfSelfLoops + "]. Default is "
                        + Globals.selfLoops + ".");
        ...
        directRecursionOption = new Option("R", "direct-recursion", true,
                "Number of direct recursive calls. Default is "
                        + Globals.directRecursiveCalls + ".");
        ...
        indirectRecursionOption = new Option("I", "indirect-recursion", true,
                "Number of indirect recursive calls. Default is "
                        + Globals.indirectRecursiveCalls + ".");
        indirectRecursionOption.setRequired(false);
        options.addOption(indirectRecursionOption);

        breaksOption = new Option("b", "breaks", false,
    ...
    private static void parseCommandLine (String[] args)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    String arg = line.getOptionValue(numberOfVerticesOption
                            .getOpt());
                    try
                    {
                        ...
                        if (vertices < minVertices || vertices > maxVertices)
                        {
                            throw new IllegalArgumentException(
                                    vertices
                                            ...
                                            + "It must be a positive integer in the range ["
                ...
                if (line.hasOption(fanOutOption.getOpt()))
                {
                    ...
                    catch (IllegalArgumentException e)
                    {
                        System.err
                                .println(arg
                                        + " is not a valid fan out. It must be in the range ["
                                        + minFanOut + ".." + maxFanOut + "].");
                        System.exit(1);
                    }
                }
                ...
                {
                    String arg = line
                            .getOptionValue(subprogramsOption.getOpt());
                    try
                    {
                ...
                {
                    String arg = line.getOptionValue(directRecursionOption
                            .getOpt());
                    try
                    {
                ...
                {
                    String arg = line.getOptionValue(indirectRecursionOption
                            .getOpt());
                    try
                    {
                        int calls = Integer.parseInt(arg);
