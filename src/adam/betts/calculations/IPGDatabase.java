...
public class IPGDatabase extends Database
{
	...
	public IPGDatabase (final Program program)
	{
		...
		ControlFlowGraph cfg = program.getSubprogram (Globals.getRoot ()).getCFG ();
		ipg = new IpointGraph ();
		Ipoint startv = new Ipoint (ipg.getNextVertexID (), cfg.getFirstAddress ());
		ipg.addIpoint (startv);
		Ipoint endv = new Ipoint (ipg.getNextVertexID (), cfg.getLastAddress ());
		ipg.addIpoint (endv);

		ipg.setEntryID (startv.getVertexID ());
		ipg.setExitID (endv.getVertexID ());

		TraceParser parser = new TraceParser (rootID, ipg, startv, endv);
	...
	{
		UDrawGraph.makeUDrawFile (Globals.getInstrumentationProfile (), ipg, "inlined");
	...
	private void initialise (int rootID, TraceParser parser)
	{
		...
		parser.properDescendants.put (rootID, new HashMap <Integer, HashSet <Integer>> ());
	...
	private class TraceParser
	{
		...
		private TraceParser (int rootID, IpointGraph ipg, Ipoint startv, Ipoint endv)
		{
			this.rootID = rootID;
			this.ipg = ipg;
			this.startv = startv;
		...
		private void doParsing (boolean doObservedWCET)
		{
				...
				while ((str = in.readLine ()) != null)
				{
					if (!str.startsWith ("//") && !str.startsWith ("/*"))
					{
						...
						Debug.debugMessage (getClass (),
								"Ipoint = 0x" + Long.toHexString (ipointID) + ", t = " + timeStamp,
								3);
						...
						{
							Debug.debugMessage (getClass (),
									"Unable to find successor of ipoint " + source.getVertexID ()
											+ " with ipoint ID 0x" + Long.toHexString (ipointID), 1);

							if (ipg.hasIpointID (ipointID) == false)
							{
								Ipoint newv = new Ipoint (ipg.getNextVertexID (), ipointID);
								ipg.addIpoint (newv);
							}
							...
							source.addSuccessor (destination.getVertexID (),
									destination.getIpointID (), newEdgeID, IPGEdgeType.TRACE_EDGE);
							destination.addPredecessor (source.getVertexID (), newEdgeID,
									IPGEdgeType.TRACE_EDGE);

							Debug.debugMessage (getClass (), "Adding edge " + source.getVertexID ()
									+ " => " + destination.getVertexID (), 1);

							unitWCETs.get (subprogramID).put (newEdgeID, (long) 0);
			...
			{
				long wcet = doWCET ();
				System.out.println ("WCET at trace #" + testCounter + " = " + wcet);
			}
		}
		...
		{
			if (destination.getVertexID () == endv.getVertexID ())
			{
		...
		private void commitTempBounds ()
		{
			for (int subprogramID : tempUnitBounds.keySet ())
			{
				...
				{
					int boundOnThisRun = tempUnitBounds.get (subprogramID).get (edgeID);
					if (boundOnThisRun > unitBounds.get (subprogramID).get (edgeID))
					{
						unitBounds.get (subprogramID).put (edgeID, boundOnThisRun);
						Debug.debugMessage (getClass (), "Edge " + edgeID + ", bound = "
								+ boundOnThisRun, 4);
					}
