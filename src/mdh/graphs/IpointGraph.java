...
public class IpointGraph extends FlowGraph
{
	...
	public IpointGraph (CFGStar cfgStar, LoopNests lnt)
	{
		...
		Debug.debugMessage (new CallBack ()
		{
			public void doJob ()
			{
				...
				{
					Debug.debugMessage (IpointGraph.class, "topsort("
							+ headerID + ") = "
			...
			public void doJob ()
			{
				for (int vertexID: auxiliaryData.vToReachable.keySet ())
				{
					for (int keyID: auxiliaryData.vToReachable.get (vertexID)
							.keySet ())
					{
						Debug.debugMessage (IpointGraph.class, "Path("
								+ keyID
								+ " => "
								+ vertexID
								+ ") = "
	...
	public IpointGraph (CFGStar cfgStar)
	{
		...
		Debug.debugMessage (getClass (),
				"Adding edges using data-flow analysis", 2);
	...
	{
		Debug.debugMessage (getClass (), "Looking for Ipoint "
				+ Long.toHexString (ipointID), 4);
		for (Vertex v: this)
		{
			Ipoint u = (Ipoint) v;
			/*
			...
			if (cfgStar.isIpoint (vertexID))
			{
				...
				else
				{
					...
					HeaderVertex headerv = (HeaderVertex) lnt
							.getVertex (headerID);
					auxiliaryData.ipointToHeader.put (vertexID, headerv
							.getHeaderID ());
				}
			}
			...
			if (lnt.isLoopHeader (vertexID))
			{
				if (!lnt.isSelfLoop (vertexID))
				{
					...
					while (bodyIt.hasNext ())
					{
						...
						{
							auxiliaryData.hToIpoints.get (vertexID)
									.add (bodyID);
						}
					}
				}
	...
	private void doDFS (AuxiliaryData auxiliaryData,
			Set<Integer> visited,
			int vertexID)
	{
		...
		if (vertexID != rootv.getHeaderID ())
		{
			...
			if (lnt.isLoopHeader (vertexID))
			{
				...
				Debug.debugMessage (getClass (), "Adding " + vertexID
						+ " to enclosing header "
			...
			while (levelIt.hasNext ())
			{
				...
				if (lnt.isLoopHeader (vertexID) && !lnt.isSelfLoop (vertexID))
				{
					...
					if (!cfgStar.isIpoint (vertexID))
					{
						...
						path.add (vertexID);
						auxiliaryData.vToReachable.get (vertexID).put (
								vertexID, path);
					}
		...
		while (auxiliaryData.changed)
		{
			...
			for (int vertexID: auxiliaryData.hToTopSorts.get (headerID))
			{
				...
				while (predIt.hasNext ())
				{
					...
					if (analyseEdge)
					{
						Debug.debugMessage (getClass (), "It "
								+ auxiliaryData.it + ": CFG* edge " + predID
								+ " => " + vertexID, 4);
						...
						else
						{
							...
							{
								updateLoopReachability (auxiliaryData,
										headerID, vertexID, predID);
							}
						}
					}
				}
	...
	private void update (AuxiliaryData auxiliaryData,
			int uID,
			int vID,
			int headerID,
			int predID)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					int oldSize = auxiliaryData.vToReachable.get (vID)
							.get (uID).size ();
					auxiliaryData.vToReachable.get (vID).get (uID).addAll (
							auxiliaryData.vToReachable.get (predID).get (uID));
					int newSize = auxiliaryData.vToReachable.get (vID)
							.get (uID).size ();

					if (oldSize != newSize)
					{
						Debug.debugMessage (getClass (), "Added "
								+ auxiliaryData.vToReachable.get (predID).get (
										uID) + " to the path " + uID + " to "
								+ vID, 4);
						auxiliaryData.changed = true;
					}
				}
	...
	private void addIPGEdge (AuxiliaryData auxiliaryData,
			int sourceID,
			int destinationID)
	{
	...
	private void addEdgeLabel (AuxiliaryData auxiliaryData,
			int sourceID,
			int destinationID,
	...
	private void setIterationEdge (AuxiliaryData auxiliaryData,
			int sourceID,
			int destinationID)
	{
			...
			int destinationID)
	{
		...
		{
			if (lnt.isNested (sourceHeaderv.getVertexID (), destinationHeaderv
					.getVertexID ()))
			{
	...
	private void addLoopEntryEdges (AuxiliaryData auxiliaryData,
			int sourceID,
			int headerID,
			int predID)
	{
		Debug.debugMessage (getClass (), "Adding loop entry edges of "
				+ headerID, 3);
			...
			int predID)
	{
		...
		if (cfgStar.isIpoint (vertexID))
		{
			...
			{
				int oldSize = auxiliaryData.hToReachable.get (headerID).get (
						vertexID).size ();
				auxiliaryData.hToReachable.get (headerID).get (vertexID)
						.addAll (
								auxiliaryData.vToReachable.get (predID).get (
										headerID));
				int newSize = auxiliaryData.hToReachable.get (headerID).get (
						vertexID).size ();
					...
					&& vertexID != headerID)
			{
						...
						.keySet ())
				{
							...
							ipointID))
					{
						...
						path.addAll (auxiliaryData.vToReachable.get (predID)
								.get (headerID));
						auxiliaryData.hToReachable.get (headerID).put (
								ipointID, path);
					}
					else
					{
						int oldSize = auxiliaryData.hToReachable.get (headerID)
								.get (ipointID).size ();
						auxiliaryData.hToReachable.get (headerID)
								.get (ipointID).addAll (
										auxiliaryData.vToReachable.get (predID)
												.get (headerID));
			...
			else
			{
						...
						headerID))
				{
					...
					path.addAll (auxiliaryData.vToReachable.get (predID).get (
							headerID));
					path.add (vertexID);
					auxiliaryData.vToReachable.get (vertexID).put (headerID,
							path);
				}
				else
				{
	...
	private void updateLoopExits (AuxiliaryData auxiliaryData,
			int headerID,
			int keyID,
			int predID)
	{
		...
		while (exitIt.hasNext ())
		{
			...
			{
				if (!auxiliaryData.vToReachable.get (exitID)
						.containsKey (keyID))
				{
					auxiliaryData.changed = true;
				...
				{
					int oldSize = auxiliaryData.vToReachable.get (exitID).get (
							keyID).size ();
					auxiliaryData.vToReachable.get (exitID).get (keyID)
							.addAll (
									auxiliaryData.vToReachable.get (predID)
											.get (keyID));
					int newSize = auxiliaryData.vToReachable.get (exitID).get (
							keyID).size ();
	...
	{
		Debug.debugMessage (getClass (), "Collapsing inner loops in "
				+ headerID, 3);
				...
				while (predIt.hasNext ())
				{
					...
					if (cfgStar.isIpoint (predID))
					{
						if (!vToIpoints.get (vertexID).containsKey (predID))
						{
							...
							if (!cfgStar.isIpoint (vertexID))
							{
								vToIpoints.get (vertexID).get (predID).add (
										vertexID);
							}
						}
					}
					else
					{
						...
						for (int keyID: vToIpoints.get (predID).keySet ())
						{
							if (!vToIpoints.get (vertexID).containsKey (keyID))
							{
								...
								vToIpoints.get (vertexID).get (keyID).addAll (
										pathFromKeyToPred);

								if (!cfgStar.isIpoint (vertexID))
								{
									vToIpoints.get (vertexID).get (keyID).add (
											vertexID);
								}
							}
							else
							{
								int oldSize = vToIpoints.get (vertexID).get (
										keyID).size ();
								Set<Integer> pathFromKeyToPred = vToIpoints
										.get (predID).get (keyID);
								vToIpoints.get (vertexID).get (keyID).addAll (
										pathFromKeyToPred);
								int newSize = vToIpoints.get (vertexID).get (
										keyID).size ();
