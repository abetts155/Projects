...
% ('-' * 50, '-' * 50))
        ...
        for cfg in self._program.getICFGs():
            ...
            for v in lnt:
                if isinstance(v, Vertices.HeaderVertex):
                    ...
                    self._headerBounds[(functionName, headerID)] = 0
                    self._tailBounds[(functionName, headerID)] = 0
                    self._loopBoundsInCurrentRun[(functionName, headerID)] = 0
                    for tailID in lnt.getLoopTails(headerID):
                        self._loopBoundsInCurrentRun[(functionName, tailID)] = 0
    ...
    def getHeaderBound (self, functionName, headerID):
        tupleKey = (functionName, headerID)
        assert tupleKey in self._headerBounds
        return self._headerBounds[tupleKey]
    
    def getTailsBound (self, functionName, headerID):
        tupleKey = (functionName, headerID)
        assert tupleKey in self._tailBounds
        return self._tailBounds[tupleKey]
...
class ParseTraces (TraceInformation):
        ...
        with open(tracefile, 'r') as f:
            for line in f:
                if line.startswith(newTrace):
                    ...
                    self.__computeMaximumLoopBounds()
    ...
    def __incrementLoopBounds (self):
        tupleKey = (self.__currentCFG.getName(), self.__currentBB.getVertexID())
        if tupleKey in self._loopBoundsInCurrentRun:
            self._loopBoundsInCurrentRun[tupleKey] += 1 

    def __computeMaximumLoopBounds (self):
        for headerID in self.__currentLNT.getHeaderIDs():
            tupleKey = (self.__currentCFG.getName(), headerID)
            assert tupleKey in self._headerBounds and tupleKey in self._loopBoundsInCurrentRun
            self._headerBounds[tupleKey] = max(self._headerBounds[tupleKey], self._loopBoundsInCurrentRun[tupleKey])
            self._loopBoundsInCurrentRun[tupleKey] = 0 
            tailBound = 0
            for tailID in self.__currentLNT.getLoopTails(headerID):
                tailBound += self._loopBoundsInCurrentRun[(self.__currentCFG.getName(), tailID)]
                self._loopBoundsInCurrentRun[(self.__currentCFG.getName(), tailID)] = 0
            self._tailBounds[tupleKey] = max(self._tailBounds[tupleKey], tailBound)
