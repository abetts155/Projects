...
class Region:
    ...
    def create(self, vertices: List[int]):
        shuffle(vertices)
        selected = random()
        if 1/2 < selected <= 1:
            ...
            for v in vertices[2:]:
                self.edges.append((the_entry, v))
                self.edges.append((v, the_exit))

            if len(vertices) == 3:
                self.edges.append((the_entry, the_exit))
    ...
    def connect(self, other: "Region"):
        if len(self.exits) > 1 and len(other.entries) > 1:
            ...
            self.edges.append((other_exit, the_exit))
            self.entries.extend(other.entries)
        elif len(self.exits) > 1 and len(other.exits) > 1:
            (the_entry,) = self.entries
            (other_entry,) = other.entries
            self.edges.append((the_entry, other_entry))
    ...
    def __str__(self):
        return 'E={} X={} {}'.format(' '.join(str(vertex) for vertex in self.entries),
                                     ' '.join(str(vertex) for vertex in self.exits),
